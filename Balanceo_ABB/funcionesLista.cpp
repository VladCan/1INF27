/* 
 * File:   FuncionesAuxiliares.cpp
 * Author: ANA RONCAL
 * Created on 21 de agosto de 2023, 12:11 PM
 */

#include <iostream>
#include <iomanip> /*formato de números*/
#include <fstream>
#include "Lista.h"
#include "funcionesLista.h"
using namespace std;

/*Valores iniciales de la lista*/
void construir(struct Lista & tad) {
    tad.cabeza = nullptr;
    tad.cola = nullptr;
    tad.longitud = 0;
}

/*devuelve si la lista esta vacia 1, caso contrario 0 */
const bool esListaVacia(const struct Lista & tad) {
    return tad.cabeza == nullptr;
}

/*devuelve la longitud de la lista*/
int longitud(const struct Lista  & tad) {
    return tad.longitud;
}

/*inserta un nodo siempre al inicio de la lista*/
void insertarAlInicio(struct Lista & tad, NodoArbol *elemento) {

    /*Se va cambiar por la función crear nodo*/
    struct NodoCola * nuevoNodoCola = new struct NodoCola;
    nuevoNodoCola->elemento = elemento;
    nuevoNodoCola->siguiente = tad.cabeza;

    tad.cabeza = nuevoNodoCola;
    tad.longitud++;
}

/*inserta un nodo siempre al final de la lista*/
void insertarAlFinal(struct Lista & tad, NodoArbol *elemento) {

    struct NodoCola * ultimoNodoCola = obtenerUltimoNodo(tad);
    struct NodoCola * nuevoNodoCola = crearNodo(elemento, nullptr);

    if (ultimoNodoCola == nullptr) /*Si la lista está vacia*/
        tad.cabeza = nuevoNodoCola; /*se inserta en la cabeza de la lista*/
    else //La lista ya tiene nodos
        ultimoNodoCola->siguiente = nuevoNodoCola;
    tad.longitud++;
}
/*inserta un nodo siempre al final de la cola*/
void insertarencola(struct Lista & tad, NodoArbol *elemento) {

    struct NodoCola * ultimoNodoCola = tad.cola;
    struct NodoCola * nuevoNodoCola = crearNodo(elemento, nullptr);

    if (ultimoNodoCola == nullptr){/*Si la lista está vacia*/
        tad.cabeza = nuevoNodoCola; /*se inserta en la cabeza de la lista*/
        tad.cola = nuevoNodoCola;
    } 
    else{ //La lista ya tiene nodos
        ultimoNodoCola->siguiente = nuevoNodoCola;
        tad.cola = nuevoNodoCola;
    }
    tad.longitud++;
}

/*Crea un nuevo nodo con los datos dados como parámetros*/
struct NodoCola * crearNodo(NodoArbol *elemento, struct NodoCola * siguiente) {
    struct NodoCola * nuevoNodoCola = new struct NodoCola;
    nuevoNodoCola->elemento = elemento;
    nuevoNodoCola->siguiente = siguiente;
    return nuevoNodoCola;
}

//Notar que esta función retorna nulo cuando la lista es vacía

/*Obtiene el último nodo de la lista*/
struct NodoCola * obtenerUltimoNodo(const struct Lista & tad) {
    struct NodoCola * ultimo = nullptr;
    struct NodoCola* recorrido = tad.cabeza;

    while (recorrido != nullptr) {
        ultimo = recorrido;
        recorrido = recorrido->siguiente;
    }
    return ultimo;
}

/*inserta un nodo en orden*/
//void insertarEnOrden(struct Lista & tad, NodoArbol *elemento) {
//
//    struct NodoCola * nodoAnterior = obtenerNodoAnterior(tad, elemento);
//    struct NodoCola * nuevoNodoCola = crearNodo(elemento, nullptr);
//    /*Si nodoAnterior es igual a nulo, la lista está vacia, se debe
//     insertar al inicio, no hay nodos menores que elemento*/
//    if (nodoAnterior == nullptr) { /*la lista está vacia*/
//        nuevoNodoCola->siguiente = tad.cabeza;
//        tad.cabeza = nuevoNodoCola;
//    } else {
//        nuevoNodoCola->siguiente = nodoAnterior->siguiente;
//        nodoAnterior->siguiente = nuevoNodoCola;
//    }
//    tad.longitud++;
//}

/*retorna el anterior al que tiene el elemento*/
//struct NodoCola * obtenerNodoAnterior(const struct Lista & tad, NodoArbol *elemento) {
//    struct NodoCola * anterior = nullptr;
//    struct NodoCola * recorrido = tad.cabeza;
//    while ((recorrido != nullptr) and (recorrido->elemento.elemento < elemento.elemento)) {
//        anterior = recorrido;
//        recorrido = recorrido->siguiente;
//    }
//    return anterior;
//}
//
//struct NodoCola * seEncuentra(const struct Lista & tad, NodoArbol *elemento) {
//    struct NodoCola * recorrido = tad.cabeza;
//
//    /*retorna el nodo correspondiente al elemento buscado*/
//    while ((recorrido != nullptr) and (recorrido->elemento.elemento != elemento.elemento)) {
//        recorrido = recorrido->siguiente;
//    }
//    return recorrido;
//}

NodoArbol *retornaCabeza(const struct Lista & tad){
    return tad.cabeza->elemento;
}


//void eliminaNodo(struct Lista & tad, NodoArbol *elem) {
//    struct NodoCola * ultimo = nullptr;
//    struct NodoCola * recorrido = tad.cabeza;
//    /*Avanzo hasta encontrar el elemento*/
//    /*Si no lo encuentra no elimina nada*/
//    while ((recorrido != nullptr){
//        int a=recorrido->elemento->elemento;
//        int b=elem->elemento;
//        if(a==b) break;
//        ultimo = recorrido;
//        recorrido = recorrido->siguiente;
//    }
//
//    if (recorrido != nullptr) {
//        if (ultimo == nullptr) /*Estoy al inicio de la lista*/
//            tad.cabeza = recorrido->siguiente;
//        else
//            ultimo->siguiente = recorrido->siguiente;
//        delete recorrido; /*libera la memoria*/
//        tad.longitud--;
//    }
//}

void eliminaCabeza(struct Lista & tad) {
    struct NodoCola * nodoAEliminar = tad.cabeza;
    if (nodoAEliminar != nullptr) {
        tad.cabeza = tad.cabeza->siguiente;
        delete nodoAEliminar;
        tad.longitud--;
    }
}

/*Elimina el último elemento de la lista, es llamado cola*/
void eliminaCola(struct Lista & tad) {
    struct NodoCola * ultimo = nullptr;
    struct NodoCola * penultimo = nullptr;
    struct NodoCola * recorrido = tad.cabeza;

    while (recorrido != nullptr) {
        if (ultimo != nullptr)
            penultimo = ultimo;
        ultimo = recorrido;
        recorrido = recorrido->siguiente;
    }

    if (ultimo != nullptr) {
        //Si no hay penultimo, se está¡ eliminando el Único nodo de la lista
        if (penultimo == nullptr)
            tad.cabeza = nullptr;
        else
            penultimo->siguiente = nullptr;
        delete ultimo;
        tad.longitud--;
    }
}

void destruir(struct Lista & tad) {
    /*recorrido apunta al inicio del tad*/
    struct NodoCola * recorrido = tad.cabeza;

    while (recorrido != nullptr) {
        /*NodoCola auxiliar que va servir para eliminar los nodos*/
        struct NodoCola * nodoAEliminar = recorrido;
        recorrido = recorrido->siguiente;
        delete nodoAEliminar;
    }
    /*la lista queda vacia*/
    tad.cabeza = nullptr;
    tad.longitud = 0;
}

/*Se recorre la lista en forma secuencial*/
/*se utiliza para no modificar la estructura, la prOteje de ti mismo*/
/*Si se quiere acceder a los datos, por ejemplo tad.longitud = 20, no te deja
 dado que la estructura es de lectura solamente*/

/*con const aquello que pasas no va cambiar, el compilador se da cuenta que es constante*/
void imprime(const struct Lista & tad) {
    cout << fixed;
    cout << setprecision(2);
    if (esListaVacia(tad)) {
        cout << "La lista está vacía no se puede mostrar" << endl;
        return;
    } else {
        struct NodoCola * recorrido = tad.cabeza;
        int estaImprimiendoLaCabeza = 1;
        cout << "[";

        while (recorrido != nullptr) {
            /*Este artificio coloca la primera coma después de la cabeza*/
            if (!estaImprimiendoLaCabeza)
                cout << ", ";
            estaImprimiendoLaCabeza = 0;
            cout << recorrido->elemento->elemento;
            recorrido = recorrido->siguiente;
        }
        cout << "]" << endl;
    }
}