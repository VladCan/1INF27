...
struct Nodo * obtenerNodoAnteriorUltimo(const struct  Lista &tad){
    struct Nodo * ultimo = nullptr;
    struct Nodo* recorrido = tad.cabeza;
...
void invertirLista(struct Lista & tad,int longitud){
    struct Nodo *ultimo;
    struct Nodo *antUltimo;
    if(longitud==1 or longitud==0){
        return;
    }else{
        ultimo=obtenerUltimoNodo(tad);
        antUltimo=obtenerNodoAnteriorUltimo(tad);
        ultimo->siguiente=antUltimo;
        antUltimo->siguiente=nullptr;
        invertirLista(tad,longitud-1);
    }
    tad.cabeza=ultimo;
